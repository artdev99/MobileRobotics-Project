...
async def turn_off_all_leds():
    async with thymiodirect.ThymioAsyncClient() as client:
        ...
        await client.set_variable("leds.bottom.left", [0, 0, 0])  # Turn off the left bottom LED
        await client.set_variable("leds.bottom.right", [0, 0, 0])  # Turn off the right bottom LED
        await client.set_variable("leds.circle", [0]*8)      # Turn off all the circle LEDs
        await client.set_variable("leds.prox.horiz", [0]*5)  # Turn off proximity sensor LEDs
        await client.set_variable("leds.prox.grids", [0, 0])  # Turn off the ground sensor LEDs
