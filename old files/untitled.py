    ...
    async def main():
        async with client.lock() as node:
            ...
            while True:
                ...
                if Path_planning:
                    if (
                        Thymio.target_keypoint == None
                    ):  # only possible at first iteration to not take time later
                        do_plot = True
                    ...
                    keypoints = find_keypoints(
                        path_img, ANGLE_THRESHOLD, STEP, COUNTER_THRESHOLD
                    )
                    keypoints = keypoints[
                        np.linalg.norm(keypoints - Thymio.xytheta_est[:2], axis=1)
                        < keypoint_dist_thresh,
                        :,
                    ]  # Keep only far keypoints
                ...
                if (step % 5) == 0:
                    x_cm, y_cm, theta_rad, x_goal_cm, y_goal_cm = adjust_units(
                        Thymio.xytheta_meas, Thymio.target_keypoint, cam.pixbymm
                    )
                ...
                if obstacle:
                    ...
                    # Target Achieved?
                        ...
                        if (
                            np.linalg.norm(
                                Thymio.xytheta_est[:2] - Thymio.target_keypoint
                            )
                            < keypoint_dist_thresh
                        ):
