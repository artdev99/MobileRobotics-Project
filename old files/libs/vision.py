...
def get_image_live(
    ...
    while True:
        ...
        temp = frame.copy()
        temp = cv2.resize(
            temp, None, fx=resize_factor, fy=resize_factor, interpolation=cv2.INTER_AREA
        )

        # edges = feature.canny(cv2.cvtColor(temp, cv2.COLOR_BGR2GRAY), sigma=sigma)
        blurred_image = cv2.GaussianBlur(
            cv2.cvtColor(temp, cv2.COLOR_BGR2GRAY), (sigma, sigma), 1.4
        )
...
def find_corners(
    lcc_mask: np.ndarray, epsilon: float, eps_security=True, verbose=True
) -> np.ndarray:
    """
    ...
    contours, _ = cv2.findContours(
        (lcc_mask * 255).astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
    largest_contour = max(contours, key=cv2.contourArea)

    approx = cv2.approxPolyDP(
        largest_contour, epsilon * cv2.arcLength(largest_contour, True), True
    )
...
def correct_perspective(
    ...
    epsilon=0.01,
    M=0,
    max_width_perspective=0,
    max_height_perspective=0,
    get_matrix=True,
    aruco=True,
    eps_security=True,
    verbose=False,
    ...
    if get_matrix:
        if aruco:
            ...
            corners = find_corners(
                mask, epsilon=epsilon, eps_security=eps_security, verbose=verbose
            )
        ordered_corners = order_points(corners)
        max_width_perspective, max_height_perspective = compute_destination_size(
            ordered_corners
        )
    ...
    corrected_image = cv2.warpPerspective(
        image,
        M,
        (max_width_perspective, max_height_perspective),
...
def full_detection_cnt_centroid(
    image: np.ndarray, thresh_obstacle, thresh_goal, min_size
) -> np.ndarray:
...
def get_dominant_object_grid(block: np.ndarray, verbose: bool) -> int:
    pixels = block.reshape(-1, 3)
    has_red = np.any(np.all(pixels == RED, axis=1))
...
def discretize_image(image: np.ndarray, grid_size=100, verbose=True, full_output=False):
    """
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            ...
            dominant_object = get_dominant_object_grid(block, verbose)

            grid[i, j] = dominant_object  #
            if full_output:
                discretized_image[start_y:end_y, start_x:end_x] = get_color(
                    dominant_object
                )
...
def discretize_image_eff(image, grid_size):
    ...
    mask = np.all(image == RED, axis=2).astype(np.uint8)
...
def get_centroids(grid: np.ndarray, _object):
    ...
    if isinstance(_object, str):
        ...
        is_image
        and isinstance(_object, (list, tuple, np.ndarray))
        and len(_object) == 3
    ):
...
def find_nose_corners(
    ...
    contours, _ = cv2.findContours(
        curved_border, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE
    )
    if not contours:
        return None, None
...
def get_slope_intercept(point1, point2):
    if point1[0] == point2[0]:
        raise ValueError(
            f"Vertical line detected, cannot compute slope = ({point1}, {point2})"
        )
...
def get_nose(
    ...
    curve, bp1, bp2 = find_nose_corners(
        image,
        t1=t1,
        t2=t2,
        sigma=sigma,
...
def init(
    ...
    image, mat_persp, max_width_persp, max_height_persp, size_aruco = (
        correct_perspective(
            image, sigma=sigma, epsilon=epsilon, get_matrix=True, aruco=aruco
        )
    )
    ...
    (
        ...
        Goal_center,
        Thymio_xytheta,
    ) = full_detection_cnt_centroid(image, thresh_obstacle, thresh_goal, min_size)
    # grid_image = grid_to_image(grid)
    grid = discretize_image_eff(thresholded_image, grid_size)
    ...
    path, _, cost_map = a_star_search(
        grid,
        grid1_coord2grid2_coord(
            np.array([Thymio_xytheta[1], Thymio_xytheta[0]]), image, grid
        ),
        grid1_coord2grid2_coord(
            np.array([Goal_center[1], Goal_center[0]]), image, grid
        ),
    )
...
def Thymio_position(img, thresh_Thymio, Thymio_size):
    ...
    if Thymio_size < 0:
        cnt, _ = cv2.findContours(white_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        cnt = sorted(
            cnt, key=cv2.contourArea, reverse=True
        )  # sort by largest cnt, there should be only one with small blob removal but we never know :)
    ...
    intersection = cv2.bitwise_and(
        box_mask, ~Thymio_mask
    )  # (box_mask & ~Thymio_mask)*255 manual but less opti
...
def update_vision(
    cam, sigma=5, epsilon=0.01, mat_persp=0, max_width_persp=0, max_height_persp=0
):

    image = get_image_from_camera(cam, False)  # camera calibration inside

    image, _, _, _, _ = correct_perspective(
        image,
        sigma=sigma,
        epsilon=epsilon,
        M=mat_persp,
        max_width_perspective=max_width_persp,
        max_height_perspective=max_height_persp,
        get_matrix=False,
    )
    ...
    return image, Thymio_xytheta, Thymio_detected
...
def draw_cnt_image(
    ...
    if Thymio_detected:
        Thymio_nose = (
            1.5
            * aruco_size
            * np.array([np.cos(Thymio_xytheta[2]), np.sin(Thymio_xytheta[2])])
        )
        Thymio_nose = Thymio_nose + Thymio_xytheta[:2]
        cv2.arrowedLine(
            image_cnt,
            Thymio_xytheta[:2].astype(int),
...
def Thymio_position_aruco_init(img):
    ...
    if (ids is None) or (Thymio_ID not in ids):
