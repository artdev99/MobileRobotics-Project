...
KFilter.F = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])  # State transition matrix

KFilter.H = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])  # Measurement function
...
def run_Kalman_filter(
    right_wheel_speed, left_wheel_speed, previous_angle, vision_measurements
):
    global KFilter
    ...
    Wheels_speed = np.array([[right_wheel_speed], [left_wheel_speed]])
    Transition = (
        np.array(
            [
                [
                    np.cos(previous_angle) * (delta_t / 2),
                    np.cos(previous_angle) * (delta_t / 2),
                ],
                [
                    np.sin(previous_angle) * (delta_t / 2),
                    np.sin(previous_angle) * (delta_t / 2),
                ],
    ...
    if vision_measurements is not None:
        measurement = np.array(
            [
                vision_measurements[0] / scale_camera,
                vision_measurements[1] / scale_camera,
                vision_measurements[2],
            ]
        )
        KFilter.update(measurement)

    return np.array(
        [KFilter.x[0] * scale_camera, KFilter.x[1] * scale_camera, KFilter.x[2]]
    )
