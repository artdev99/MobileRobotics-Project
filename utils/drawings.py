...
def draw_on_image(camera, Thymio, path_img):
    ...
    #Thymio's measured position (camera)
    radius = 1.5 * camera.size_aruco
    if Thymio.Thymio_detected:
        Thymio_nose = radius * np.array([np.cos(Thymio.xytheta_meas[2]), np.sin(Thymio.xytheta_meas[2])])
    ...
    # Thymio's estimated position(Kalman)
    # 2 sigma-confidence Position (95%)
    ...
    # Start of the arc:
    start_angle = np.max([np.degrees(Thymio.xytheta_est[2]) - np.degrees(2 * np.sqrt(Thymio.kalman_P[2, 2])),0])
    # End of the arc
    end_angle = np.min([np.degrees(Thymio.xytheta_est[2]) + np.degrees(2 * np.sqrt(Thymio.kalman_P[2, 2])),360])
